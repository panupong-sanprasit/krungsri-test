...
Following instructions explain how to publish krungsri-test tutorial APIs via a portable Docker container.
...
- krungsri-test project sources locally compiled, and shell pointing to project root.


### Build image
`mvn clean package docker:build`

Now if you issue a `docker images` command you should see `krungsri-test:latest` image listed.
...
#### From command line
`docker run -d -p <SERVER_PORT>:8080 krungsri-test`
